  ...
  try {
    ...
    if (!apiKey) {
      return res.status(500).json({
        error: "Missing DIFY_API_KEY in Vercel Environment Variables."
      });
    }
    ...
    if (!difyResponse.ok) {
      ...
      return res
        ...
        .json(safeJsonParse(errorText) || { error: errorText });
    }

    if (!difyResponse.body) {
      return res.status(500).json({ error: "Dify response body is empty" });
    }
    ...
    while (true) {
      ...
      for (const chunk of chunks) {
        ...
        if (eventData.event === "text_chunk" && eventData.data?.text) {
          finalText += eventData.data.text;
        }

        if (eventData.event === "message" && eventData.answer) {
          finalText += eventData.answer;
        }
      }
    }
